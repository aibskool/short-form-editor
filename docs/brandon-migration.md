...
| Legacy `graphic`, `scene`, `labels`, `output.split_fraction`, `music`, `audio_policy.user_opt_out` | Continue to render with their earlier meaning. Do not automatically convert conceptual illustrations or source labels into editorial claims. `editorial_graphics` is authored separately with claim IDs. |
| `audio_policy.music_required` build gate and previous unconditional music expectation | A required bed still requires a real track. For intentional silence, set `music_required:false` with `music_free_reason`; the legacy `user_opt_out` is accepted. The runner and direct editor build enforce the explicit policy. |
| `production/editor/package.json` and lockfile package name | `brandon-reel-editor`; renderer dependency pins retained. |
| `production/editor/edit.py`, `motion_scenes.py`, `editorial_scenes.py`, `timeline.example.json` | New caption defaults and green/white graphic overlays; neutral demonstration labels. The old Council shot structure is not the Brandon default. |
...
| Migration | Dry run, repeat run, target conflict and preserved legacy timing/keys | Automated tests pass. |
...
| Audio and motion | Recorded voice intact; music policy explicit; visual changes based on meaning; effect levels checked by listening | Structural checks pass; encoded listening pending. |
| Calibration | Exactly 20 seconds of original Brandon footage, render hash, every encoded frame compared with selected intervals of all three source reels and audio listened to | **Pending** original Brandon footage, authorized local reference files, Chrome renderer and Brandon review. `production/compare_style.py` emits `pending_human_review`, never a match score. |
