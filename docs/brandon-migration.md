...
| `production/editor/edit.py`, `motion_scenes.py`, `editorial_scenes.py`, `kinetic_scenes.py`, `timeline.example.json` | New caption defaults, green/white animated scenes, purpose-timed transitions, no-music build gate and neutral demonstration labels. The old Council shot structure is not the Brandon default. |
