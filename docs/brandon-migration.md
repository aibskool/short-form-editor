...
| `production/editorial-map.schema.json`, `asset-index.schema.json`, `check_editorial.py`, `retime_timeline.py` | Legacy `truth_status`, `proof_scope`, `evidence_refs`, `limitations`, `on_screen_label`, and `evidence_honesty` remain accepted but are optional. A `proof` visual may be an authored UI screen. Structural joins and keyword/resource checks remain, with no claim or source gate; retime both text systems. Existing schema version 1 data remains valid. |
...
5. Existing research banks with `sources` and editorial maps with proof metadata may be left as they are. New banks need only stable story IDs and titles for research readiness. For new editorial maps, omit the optional proof fields and score the four active review dimensions; historical values are still accepted. This migration does not rewrite historical examples or their attribution.
