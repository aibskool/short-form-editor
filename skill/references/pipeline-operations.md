...
## Brandon's claim and graphic policy

Take Brandon's stated account and recorded claims as the script input for his reels. Do not run claim-accuracy checks, demand supporting evidence, or reject a beat for absent proof at research, scripting, edit or release. Source-status fields below are optional asset and attribution notes. When a screen capture does not show a stated purchase or sent email, create an animated confirmation or sent-email screen if it helps the story. Do not add proof-status badges, disclaimers or production caveats to the video. Animate every authored on-screen element and add purposeful transitions at appropriate cuts even when the assembly omits them.

Turn Brandon's story into a reel he can film, then edit his supplied footage into a finished video. Before filming, deliver the spoken script, visual requirements and promised resource. Source links may be included for asset discovery or attribution. After filming, deliver the rendered MP4 plus its editable timeline, indexed assets and giveaway handoff.

**Choose visuals by the spoken beat.** Use supplied captures when they show the action and animated authored screens or graphics when a step was not captured. Original posts, documentation and illustrative footage can add useful context. Choose each shot for the viewer's question and the spoken claim it explains. Read [visual-evidence.md](visual-evidence.md) for asset provenance and framing.
...
Curated resource reels are an optional format, based on Samin's historical five-resource reference: one viewer outcome, several complementary plugins/skills/tools, an outcome-first hook and one complete-resource CTA. Use other formats when Brandon's story calls for them.

| Stage | Input | Saved output | Ready when |
|---|---|---|---|
| Research | Brandon's account, optional saves and visual ideas | `research/story-bank.json`, optional visual index and shot queue | Stable story ID and title exist; no source-list or claim check |
| Ideation | Story bank and voice examples | `ideation/ideas.json` and `ideation/ideas.md` | Each selected idea has an angle, hook options, keyword and deliverable promise |
| Scripting | Selected idea and matching voice samples | `scripts/*.md`, `reel-manifest.json`, optional shot queue | Clean spoken copy matches Brandon's account and the selected idea; visual beats identify what viewers should see and why |
...
When saved resources may help, read [saved-resources.md](saved-resources.md) and search Brandon's authorized collection. Preserve item IDs, original URLs, what was actually read and any reuse decision. Lack of Eden access does not block writing or editing Brandon's account; never claim an inspection that did not happen.
...
3. For useful third-party visuals, open the original page and retain author, dates and identity in the unaltered master. Record the URLs in optional [research notes](research-contract.md) and start an optional shot queue using [visual-evidence.md](visual-evidence.md). An uncaptured step in Brandon's story can be communicated with an animated authored screen. A URL alone is a request for that specific sourced asset, not a required story credential.

4. Compare against the corpus and `<project>/story-ledger.json`. If that ledger is missing, create it from prior run manifests; do not infer publication. Distinguish reference/filming, drafted, accepted, and published. Rank by visible payoff, clarity of mechanism and audience fit; a source-strength score is not used.
...
For each promising story, propose two or three genuinely different hook angles. Select one using viewer usefulness, the visual demonstration and fit with the source corpus. The recommendation can be made autonomously unless the user requested a selection pause.
...
- For an optional resource stack, use the reference's outcome-first count hook, ordered named resource beats, distinct practical uses and complete-list CTA. Follow [curated-resource-reels.md](curated-resource-reels.md) for cadence and the hook/body/visual/giveaway count check. Brandon's recorded story takes precedence.
...
- Write one spoken beat per paragraph. Keep stage directions, optional source URLs and timings in editor notes. Do not insert disclaimers or production caveats into spoken or on-screen copy.
...
- Beside the clean script, attach each visual beat to a viewer question, visual job, exact spoken noun/action and required reading hold. Use supplied screen captures when useful. Author animated confirmations, sent-email screens and explanatory graphics for uncaptured steps in Brandon's stated account. Keep their provenance in production notes without putting labels or caveats in the picture.
...
1. Check that the spoken copy, captions and graphic wording match Brandon's account. Do not run a claim-accuracy or supporting-evidence check.
...
4. Save a filming document containing the spoken scripts, words/timing estimates, visual plan and matching resource paths. Each planned shot needs a reason and readable focal detail. Optional source URLs and capture status may help production but cannot block editing. Preserve clean individual scripts for filming and mechanical checks. Use Roughdraft when the user wants review/comments; wait for Done Reviewing, then read the edited file and address CriticMarkup.
...
- Promote any useful shot queue into the asset index and `editorial-map.json`. Use supplied screen captures, A-roll, illustrative footage and animated authored UI according to the spoken beat. Record asset origins, ownership/reuse, hash and capture or creation method in production notes. A constructed purchase or sent-email screen may represent Brandon's stated event without a capture or on-screen caveat. Do not pass an unrelated screen off as a supplied capture.
...
`research/story-bank.json`, `ideation/ideas.json`, `scripts/*.md`, `assets/<KEYWORD>.md`, `skool/drafts/`, both delivery handoffs, a combined review document and a receipt listing what was drafted and delivered. Optional saved-resource, visual-index and shot-queue files help when relevant. Editing adds the asset index, `editorial-map.json`, timeline and inspected render. Keep sources and samples private unless the user asks to share them. `pipeline.py` checks stage joins and ManyChat delivery fields, not truth or supporting evidence; separately inspect Skool paths, asset hashes, attribution and access before calling the full handoff complete.
