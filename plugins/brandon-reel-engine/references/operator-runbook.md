...
Brandon's supplied reels combine original face-to-camera delivery, short lower-third spoken captions, a separate green/white editorial graphic system, real screen proof, contextual footage, and selected music/effect accents. Select a treatment by the spoken claim. Music is required when the edit's recorded audio plan sets `music_required:true`; a silent plan must be intentional. Record the opening, both text tracks, assets and actual review observations in `creative-review.json` using [quality procedure](quality-procedure.md).
...
Record `audio_policy.music_required` for the selected edit; when true, add and validate actual music. When false, record the reason in `audio_policy.music_free_reason`. Copy `<plugin>/templates/creative-review.json` to the pilot directory and fill observed fields as the work proceeds.
