...
Lock the edited words first. Fill the editorial map before assembling: spoken claim → viewer question → visual job → evidence/asset → important word → reveal → reading hold → expected takeaway. Use `production/editorial-map.schema.json`; the Council map is a historical upstream example, not Brandon's style template. Every word must belong to a beat; one shot can cover multiple related beats. Read [Brandon's observed profile](brandon-style-profile.md) before choosing treatments.
