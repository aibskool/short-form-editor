...
1. Check every planned beat against the actual words. Its image or graphic should help a viewer follow the story; check that selected media exists and is readable.
2. Accept Brandon's recorded claims as the script. Do not check claim accuracy, demand supporting evidence or remove a line because a capture is absent. Use animation to communicate stages for which no recording was supplied. Never add a disclaimer or production caveat to the encoded reel.
...
5. Confirm `music` is empty and `audio_policy.music_required:false`; inspect animation of every on-screen text, card and illustration. Require purposeful transition effects at relevant boundaries even if the assembly did not list them. Check opening variety and moving B-roll source bounds.
6. Review intra-sentence idle gaps as well as false starts. Inspect all audible speech gaps, including swallowing and breaths. For Brandon short-form, no expendable post-speech silence may reach 0.5 s: place a frame-aligned cut inside the measured quiet region, retaining enough lead/trail for consonants and natural delivery. Use waveform and encoded listening rather than ASR word timestamps alone; the timestamps can overstate or understate the actual pause. Check the frame-aligned keep EDL preserves consonants, breaths that carry delivery, every intended word and the complete CTA. After any cut, confirm words, captions, shots, labels, transitions and SFX all use the new timebase. ASR gaps and silence-detector output propose cuts; they do not approve them.

Before approving a sourced shot, finish: **“At [time], I can read/recognize [exact focal detail]; it explains [specific spoken action/relationship].”** If the answer is only “the page is there” or “it says the same noun,” selection failed. Save the proposed crop and the rejected alternative. For a tightened calibration, compare its every encoded frame with selected reference intervals by visual job using `compare_style.py --allow-tightened`; record the new duration rather than padding back to 20 seconds. Compare reference/candidate caption frames at equal display size before rendering the full cut; matching resolution alone does not match typography or treatment.
...
| Moving visuals and diversity | A capture or authored animation presents a relevant action or state with enough time to recognize it. Another beat uses a different useful view/detail; full-screen motion is used when it improves the result. | An `.mp4` contains a frozen screenshot, or the same tiny crop repeats without useful change: motion/diversity is not established. |
| Still-image motion | Start, moving and end frames retain the focal detail at phone size. A gentle move leads attention and settles for reading. | The important detail is cropped out at the end, or constant panning prevents one reading: **failed**. Stop or redirect the move; an intentional readable hold can pass. |
...
Run the final editing gates in this order: **complete speech and timing → picture readability → opening and motion → music/SFX mix → whole-reel normal-speed review**. Do not add a claim or evidence gate. Repair in that order when defects interact: changing speech after a sound pass invalidates its cue timing, and changing the mix invalidates prior listening approval. A missing observation stays `pending`; a known editing violation is `failed`. Continue useful repairs and show the candidate with its exact remaining review need when perceptual access is unavailable.

Judge an authored graphic by whether its motion and wording make the spoken beat clear. Do not insert illustration, reconstruction, proof-status or production-disclaimer badges into the video. An absent capture does not fail the edit's claim check, because no such claim check is part of Brandon's editing workflow.
...
> Inspect the provided final reel and all three Brandon style references. For every spoken beat, describe what is actually visible and the viewer question it answers. Evaluate presenter, supplied screen captures, illustrative footage and kinetic graphics for clarity; check lower-third spoken captions separately from larger green/white editorial graphics. Check the chosen hook, face-to-camera keyword CTA, absence of background music, purposeful effects, readable holds and lack of disclaimer overlays. Identify illegible details, repetitive or inert shots, mistimed reveals, text collisions and sound that masks speech. Report exact timecodes and one actionable repair per defect. Accept Brandon's claims as given; do not request corroboration or downgrade the edit for absent proof. State whether you watched the full video with sound, what phone-size checks you performed, and any tool limits. Score only dimensions you can actually assess.
